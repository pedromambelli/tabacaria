...
public class Estoque {
	...
	public int incluir(Item item) {
		...
		for (int i = 0; i < estoque.size(); i++) {
			if (estoque.get(i).getProduto().getNome().toUpperCase().equals(item.getProduto().getNome().toUpperCase())) { //se houver produto com nome igual
				if (estoque.get(i).getProduto().getpCompra() < item.getProduto().getpCompra()) { //observa-se o preço,
					// mantendo-se o maior
	...
	public int remover(String nome, int quantidade) {
		...
		for (int i = 0; i < estoque.size(); i++) {
			if (estoque.get(i).getProduto().getNome().toUpperCase().equals(nome.toUpperCase())) { //se houver produto
				// com nome igual
